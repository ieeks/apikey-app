  ...
  useEffect(() => {
    return onAuthStateChanged(auth, (u) => {
      ...
      if (!ALLOWED_EMAILS.includes(u.email)) {
        signOut(auth);
        setError("Kein Zugriff — dieser Account ist nicht berechtigt.");
        setUser(null);
        return;
      }
      setUser(u);
  ...
  if (!user) {
    return (
      <div className="app auth-wrap">
        <div className="auth-card">
          <img src={islaLogo} alt="Isla" className="auth-logo" />
          <strong className="auth-title">Isla</strong>
          <p className="auth-hint">API Vault</p>
